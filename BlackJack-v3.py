...
def blackjack_hand_result(deck, bet=10, player_hand='q', dealer_card='q', hard_code= 4):
    ...
    #print('dealers card = ', dealer_cards,' players card = ', player_hand)
    
    
    ## randomly deleting other cards to simulate other players being present
    #for i in range(10):
    #        if len(deck)>0:
    #            deck = np.delete(deck,0)
    ...
    if hard_stay==False:
        ...
        if type(hard_code)== str:
            if hard_code=='Hit':
                ...
                deck = np.delete(deck,0)
                #print('dealers card = ', dealer_cards,' players card = ', player_hand)
                return blackjack_hand_result(deck, bet, player_hand, dealer_card, hard_code= 4)
            ...
            if hard_code=='Split':
                ...
                if len(player_hand)==2: 
                    ...
                    if player_hand[0]==player_hand[1]:
                        ...
                        #print('dealers card = ', dealer_cards,' players card = ', player_hand)
                        return blackjack_hand_result(deck,bet=bet, player_hand= player_hand, dealer_card=dealer_card)
...
def blackjack_sim(n_hands=5, player_h='q', dealer_c='q', starting_bet=10, hard_c=4, num_decks = 6, shuffle_perc = 50):
    ...
    for i in range(n_hands):
        #print("bet = ",bet2)
        hand_result, deck_to_play = blackjack_hand_result(deck = deck_to_play,bet=bet2, player_hand=p[:q], dealer_card=dealer_c, hard_code= hard_c)
        ...
        if pnl < lowest_running_total:
            lowest_running_total = pnl
        
    return pnl #, lowest_running_total


blackjack_sim(n_hands=100, starting_bet=5) 

test = blackjack_sim(n_hands=10, starting_bet=5) 





blackjack_sim(n_hands=50, starting_bet=5) 

blackjack_sim(n_hands=10, starting_bet=5) 




for i in range(500):
    results = blackjack_sim(n_hands=1000, starting_bet=5) 
    print(results)
...
house_edge=-100*(blackjack_sim(n_hands=100_000_000, starting_bet=1) / 1_000_000)
